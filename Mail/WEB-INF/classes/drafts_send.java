...
public class drafts_send extends HttpServlet implements SingleThreadModel
{
	...
	public void doPost(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException
   	{
		try
		{
			...
			{
				response.sendRedirect("http://localhost:8080/Mail/servlet/drafts");
			}
			else if((request.getParameter("send")+"").equals("  Send  "))
			{
				...
				{
					url="/servlet/subjectnulld";
					rd=context.getRequestDispatcher(url);
					rd.forward(request,response);
				}
				else if(request.getParameter("message").equals(""))
				{
					url="/servlet/messagenulld";
					rd=context.getRequestDispatcher(url);
					rd.forward(request,response);
				}
				else
				{
					...
					if(flag==true)
					{
						...
						rs.deleteRow();
						response.sendRedirect("http://localhost:8080/Mail/servlet/start1d");
					}
					else
					{
						stmt=con.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_UPDATABLE);
						...
						rs.deleteRow();
						response.sendRedirect("http://localhost:8080/Mail/servlet/start2d");
					}
				}
			}
			else if((request.getParameter("save")+"").equals("  Save  "))
			{
				...
				else
				{
						...
						rs.updateString("Username",user);
						rs.updateRow();
						response.sendRedirect("http://localhost:8080/Mail/servlet/start3d");
				}
			}
