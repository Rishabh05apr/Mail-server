...
public class sent1 extends HttpServlet implements SingleThreadModel
{
	String url;
	PrintWriter out;
	String check,user;
	...
	public void doPost(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException
   	{
		try
		{
			...
			{
				response.sendRedirect("http://localhost:8080/Mail/servlet/sent_deleteall");	
			}
			else if(request.getParameter("rb")==null)
			{
				response.sendRedirect("http://localhost:8080/Mail/servlet/sent_select");	
			}
			else if((request.getParameter("content")+"").equals("Content"))
			{
				url="/servlet/sent_content";
