...
public class inbox_send extends HttpServlet implements SingleThreadModel
{
	...
	public void doPost(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException
   	{
		try
		{
			...
			{
				response.sendRedirect("http://localhost:8080/Mail/servlet/inbox");
			}
			else if((request.getParameter("send")+"").equals("  Send  "))
			{
				...
				{
					url="/servlet/subjectnulli";
					rd=context.getRequestDispatcher(url);
					rd.forward(request,response);
				}
				else if(request.getParameter("message").equals(""))
				{
					url="/servlet/messagenulli";
					rd=context.getRequestDispatcher(url);
					rd.forward(request,response);
				}
				else
				{
					...
					if(flag==true)
					{
						...
						rs.insertRow();
						response.sendRedirect("http://localhost:8080/Mail/servlet/start1i");
					}
					else
					{
						stmt=con.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_UPDATABLE);
						...
						rs.updateString("Username",user);
						rs.insertRow();
						response.sendRedirect("http://localhost:8080/Mail/servlet/start2i");
					}
				}
			}
			else if((request.getParameter("save")+"").equals("  Save  "))
			{
				...
				else
				{
						...
						rs.updateString("Username",user);
						rs.insertRow();
						response.sendRedirect("http://localhost:8080/Mail/servlet/start3i");
				}
			}
